...
export class UserRepository implements UserRepositoryInterface {
  ...
  async registerWordToHistory(id: string, word: string): Promise<Word> {
    const wordDB = await WordModel.findOne({ word })
    if (wordDB === null) throw new Error(`The word ${word} does not exist in this dictionary`)
    await UserModel.findOneAndUpdate({ id }, { $addToSet: { history: wordDB._id }})
  ...
  async addWordToFavorites(id: string, word: string): Promise<Word> {
    const wordDB = await WordModel.findOne({ word })
    if (wordDB === null) throw new Error(`The word ${word} does not exist in this dictionary`)
    await UserModel.findOneAndUpdate({ id }, { $addToSet: { favorites: wordDB._id } })
    return { word: wordDB.word, added: wordDB.added }
  }
  ...
  async removeWordFromFavorites(id: string, word: string): Promise<Word> {
    const wordDB = await WordModel.findOne({ word })
    if (wordDB === null) throw new Error(`The word ${word} does not exist in this dictionary`)
    await UserModel.findOneAndUpdate({ id }, { $pull: { favorites: wordDB._id } })
    return { word: wordDB.word, added: wordDB.added }
  }

  async getHistory(id: string, pagination: PaginationQuery): Promise<{ words: Word[]; totalWords: number }> {
    ...
    const userDB = await UserModel.findOne({ id }).populate<{ history: IWord[] }>({
      path: 'history',
      options
    }).exec()
    if (userDB == null) throw new Error(`No user with id ${id} is registered`)
    const words: Word[] = userDB.history.map(word => ({ word: word.word, added: word.added }))
    return { words, totalWords: totalWords! }
  }

  async getFavorites(id: string, pagination: PaginationQuery): Promise<{ words: Word[]; totalWords: number }> {
    const page = pagination.page ?? 1
    const limit = pagination.limit
    const skip = limit === undefined ? 0 : (page-1)*limit
    ...
    const userDB = await UserModel.findOne({ id }).populate<{ favorites: IWord[] }>({
      path: 'favorites',
      options
    }).exec()
    if (userDB == null) throw new Error(`No user with id ${id} is registered`)
    const words: Word[] = userDB.favorites.map(word => ({ word: word.word, added: word.added }))
    return { words, totalWords: totalWords! }
  }
